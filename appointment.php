...
<div class="modal fade" id="editModal<?= $a['appointment_id'] ?>" tabindex="-1" aria-labelledby="editModalLabel<?= $a['appointment_id'] ?>" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <form method="post" action="appoint_crud.php">
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        <label for="patient_name<?= $a['appointment_id'] ?>">Patient Name</label>
                        <input type="text" id="patient_name<?= $a['appointment_id'] ?>" name="patient_name" class="form-control" value="<?= htmlspecialchars($a['patient_name']) ?>" required>
                    </div>
                    <div class="mb-3">
                        <label for="date<?= $a['appointment_id'] ?>">Date</label>
                        <input type="date" id="date<?= $a['appointment_id'] ?>" name="date" class="form-control" value="<?= htmlspecialchars($a['date']) ?>" required>
                    </div>
                    <div class="mb-3">
                        <label for="time<?= $a['appointment_id'] ?>">Time</label>
                        <input type="time" id="time<?= $a['appointment_id'] ?>" name="time" class="form-control" value="<?= htmlspecialchars($a['time']) ?>" required>
                    </div>
                    <div class="mb-3">
                        <label for="doctor<?= $a['appointment_id'] ?>">Doctor</label>
                        <input type="text" id="doctor<?= $a['appointment_id'] ?>" name="doctor" class="form-control" value="<?= htmlspecialchars($a['doctor']) ?>" required>
                    </div>
                    <div class="mb-3">
                        <label for="reason<?= $a['appointment_id'] ?>">Reason</label>
                        <select id="reason<?= $a['appointment_id'] ?>" name="reason" class="form-control">
                            <option value="Checkup" <?= $a['reason'] == 'Checkup' ? 'selected' : '' ?>>Checkup</option>
                            <option value="Flu" <?= $a['reason'] == 'Flu' ? 'selected' : '' ?>>Flu</option>
                            <option value="Consultation" <?= $a['reason'] == 'Consultation' ? 'selected' : '' ?>>Consultation</option>
                            <option value="Emergency" <?= $a['reason'] == 'Emergency' ? 'selected' : '' ?>>Emergency</option>
                            <?php if (!in_array($a['reason'], ['Checkup', 'Flu', 'Consultation', 'Emergency'])): ?>
                                <option value="<?= htmlspecialchars($a['reason']) ?>" selected><?= htmlspecialchars($a['reason']) ?></option>
                            <?php endif; ?>
                        </select>
                    </div>
                </div>
