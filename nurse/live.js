document.addEventListener('DOMContentLoaded', function () {
    ...
    // Add event listeners to existing remove buttons for household members
    document.querySelectorAll('.remove-member-btn').forEach(button => {
        button.addEventListener('click', function () {
            const memberDiv = this.closest('.household-member');
            if (memberDiv) {
                memberDiv.remove();
            }
        });
    ...
    if (searchInputPatient && patientTableBody) {
        searchInputPatient.addEventListener('input', function () {
            ...
            fetch(`patient.php?ajax=1&search=${encodeURIComponent(query)}`)
                .then(response => response.json())
                .then(data => {
                    patientTableBody.innerHTML = '';

                    if (data.length === 0) {
                        patientTableBody.innerHTML = '<tr><td colspan="11" class="text-center">No patients found.</td></tr>';
                        return;
                    }
